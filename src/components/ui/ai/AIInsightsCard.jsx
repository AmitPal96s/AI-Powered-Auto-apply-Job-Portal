...
import { getSuggestions } from "../../../services/ai/aiServices";
import toast from "react-hot-toast";

function AIInsightsCard({ skills = [] }) {
  ...
  const fetchSuggestions = async () => {
    ...
    setLoading(true);
    try {
      const data = await getSuggestions(skills);
      setSuggestions(data);
    ...
    <div className="bg-white rounded-3xl shadow-lg p-5 sm:p-6 transition">
      ...
      {!loading && suggestions.length === 0 && (
        <p className="text-gray-500 dark:text-gray-400 text-sm">
          No insights available. Add your skills to receive personalized
          recommendations.
        </p>
      )}
    </div>
